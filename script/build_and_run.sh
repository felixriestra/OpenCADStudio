...
sed \
  ...
  -e "s/io.github.HakanSeven12.Mac2CAM/$BUNDLE_ID/g" \
...
case "$MODE" in
  ...
  --logs|logs)
    open_app
    /usr/bin/log stream --info --style compact --predicate 'process == "Mac2CAM-App"'
    ;;
  --telemetry|telemetry)
    open_app
