//! Ported from 2DCam's `Sources/TwoDCamCore/ToolLibrary/ToolLibrarySchema.swift`.
//! The SQL is portable near-verbatim — SQLite is SQLite on either platform.

pub const VERSION: i64 = 3;

/// Executed once, on an empty database file.
pub const DDL: &str = r#"
...
-- a tool may live in several groups
...
-- estimated, inferred, or hand-edited. 'user' rows are never overwritten by a
-- catalogue re-import.
...
-- updated_at is set by the repository on every write, NOT by a trigger. A
-- trigger that rewrites updated_at re-fires the change_log trigger and writes
-- phantom no-op entries. Do not reintroduce one.
...
/// Material classes, the four vendors, and conservative fallback chipload
/// bands. The bands are PLACEHOLDERS (origin 'generic', reliability 2) and
/// the UI must present anything derived from them as an estimate.
...
/// Each entry runs inside a transaction only when the stored version is below
/// its own version number. Only `ALTER TABLE ADD COLUMN` and `CREATE TABLE`
/// shapes are used here, so nothing can lose data.
pub const MIGRATIONS: &[(i64, &str, &str)] = &[(
    3,
    "form_scale_mode",
    "ALTER TABLE tool ADD COLUMN form_scale_mode TEXT NOT NULL DEFAULT 'keepAngle';",
)];
